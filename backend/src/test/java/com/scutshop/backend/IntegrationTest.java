...
public class IntegrationTest {
        ...
        static void props(DynamicPropertyRegistry r) {
                // Allow using an existing DB (e.g. from docker-compose) by setting
                // USE_EXISTING_DB=true
                if ("true".equalsIgnoreCase(System.getenv("USE_EXISTING_DB"))) {
                        String host = System.getenv().getOrDefault("DB_HOST", "127.0.0.1");
                        String port = System.getenv().getOrDefault("DB_PORT", "3306");
                        String name = System.getenv().getOrDefault("DB_NAME", "scut_shop");
                        String user = System.getenv().getOrDefault("DB_USER", "scut_user");
                        String pass = System.getenv().getOrDefault("DB_PASSWORD", "123456");
        ...
        void init() throws Exception {
                ...
                } catch (org.springframework.jdbc.datasource.init.ScriptStatementFailedException e) {
                        Throwable c = e.getCause();
                        // If the schema already exists, ignore the error when using an existing DB
                        if (c instanceof java.sql.SQLSyntaxErrorException && c.getMessage() != null
                                        && c.getMessage().contains("already exists")) {
                                System.out.println("Schema already exists, continuing with existing DB");
                        } else {
                                throw e;
                        }
                }
        ...
        void fullCheckoutFlow() {
                ...
                verify(mailSender).send(
                                org.mockito.ArgumentMatchers.any(org.springframework.mail.SimpleMailMessage.class));
        }
}
