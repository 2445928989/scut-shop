...
> E2E 注意事项：在客户端对购物车做变更时，**前端应等待并确认 `POST /api/cart/items` 的成功响应后再在 UI 上显示“已加入购物车”**。导航或页面刷新后应调用 `GET /api/cart` 来 hydration（同步服务端购物车），以避免本地状态与服务端不一致导致 `checkout` 返回 `cart_empty` 的情况。更多调试记录与统计见 `doc/E2E-debugging.md`。
...
确认后，请在 issue 或项目面板中标注优先级并着手实施所选任务。
