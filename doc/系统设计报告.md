...
```mermaid
flowchart TB
    Browser[浏览器] --> Nginx[Nginx Port 3000]
    Nginx -->|静态文件| Vue[Vue 3 SPA]
    Nginx -->|/api 反向代理| Boot[Spring Boot Port 8080]

    Boot --> MySQL[(MySQL 8.0)]
    Boot --> Redis[(Redis 7)]
    Boot --> MailHog[MailHog SMTP]

    MySQL --> login_log[(login_log)]
    MySQL --> user_log[(user_log)]
    MySQL --> operation_log[(operation_log)]
    MySQL --> product_audit[(product_audit)]

    login_log --> Analytics[数据分析模块]
    user_log --> Analytics
    operation_log --> Analytics
    product_audit --> Analytics

    Analytics --> Forecast[销售趋势预测]
    Analytics --> CF[协同过滤推荐]
    Analytics --> CoOccur[共现商品推荐]
    Analytics --> Anomaly[异常检测]
    ...
    Vue --> Dashboard[ECharts 数据大屏]
    Vue --> Recommend[推荐商品卡片]
```
...
```mermaid
graph TB
    A[frontend Nginx] -->|API反向代理| B[app Spring Boot]
    B --> C[(MySQL 8.0)]
    B --> D[(Redis 7)]
    B --> E[MailHog SMTP]
    C --> F[(db_data持久化卷)]
...
```mermaid
flowchart TB
    subgraph cooccur["共现推荐：浏览此商品的人也买了"]
        A1[查询包含商品A的所有订单] --> A2[统计同单内其他商品的共现次数]
        A2 --> A3[计算 coPercent 百分比]
        A3 --> A4[按共现次数降序取 Top-N]
    end

    subgraph cf["协同过滤：User-CF 个性化推荐"]
        B1[构建用户-商品矩阵] --> B2[计算 Jaccard 相似度]
        B2 --> B3[选取 K 个最相似邻居]
        B3 --> B4[聚合邻居购买商品并排除已购]
        B4 --> B5[按推荐度降序输出]
    end
